...
func (s *NetServer) ConstructInitialSnapshot(existingAmbientPods []*corev1.Pod, namespaces map[string]*corev1.Namespace) error {
	...
	if s.trafficManager.ReconcileModeEnabled() {
		...
		for _, pod := range existingAmbientPods {
			...
			// NOTE: One of the big open questions I have around this change is how to handle
			// updating exclusion annotations for pods that complete reconcile. Since ConstructInitialSnapshot
			// is called on startup, would it be acceptable to update a bunch of pods annotations at this point?
